...
row = {
    ...
    "TECH_TARGET": os.environ.get("FRESHDESK_TRIAGE_TECH_GROUP_ID"),
    "CS_TARGET": os.environ.get("FRESHDESK_TRIAGE_CS_RECEIVER_ID"),
}
        ...
        (hermes_home / ".env").write_text(
            "\n".join(
                [
                    ...
                    "DWS_BIN=/runtime/dws",
                    "FRESHDESK_TRIAGE_TECH_GROUP_ID=technical-target",
                    "FRESHDESK_TRIAGE_CS_RECEIVER_ID=customer-target",
                ]
            )
            + "\n",
            encoding="utf-8",
...
def test_retryable_fail_then_success_stops_immediately_and_loads_dotenv() -> None:
    ...
    assert all(row["TECH_TARGET"] == "technical-target" for row in rows)
    assert all(row["CS_TARGET"] == "customer-target" for row in rows)
